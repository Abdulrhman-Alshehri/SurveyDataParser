...
- **Courier-Partner Attribution**: Shipments handed to a 3PL (JNT, ARAMEX, IMILE, …) are delivered by the partner's own rider, who never appears in our tracking events. Instead of leaving the rider columns blank — or mislabelling the warehouse staff who bagged the shipment as the rider — `worker_name` and `worker_phone` are set to the courier company and `worker_code` to that partner's own AWB number. SELF-fleet shipments are unaffected.
